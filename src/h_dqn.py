...
class Goal_Network( nn.Module ):
    def __init__(self, goal_size ):
        super( Goal_Network, self ).__init__()
...
class DQN_Agent():
    ...
    def __init__(self, environment_name, lr, render=False):
        ...
        self.log_file = open( "h_dqn{}-2.txt".format(TERM_STEP), "w" )
        self.test_file = open("h_dqn_test{}.txt".format( TERM_STEP ), "w" )
...
def main(args):
    ...
    if environment_name == "NEL-v0":
        ...
        agent.load_model( "h_dqn/NEL-v0250/5200/model.bin" )
    if args.train:
        agent.train()
    else:
        agent.draw()
